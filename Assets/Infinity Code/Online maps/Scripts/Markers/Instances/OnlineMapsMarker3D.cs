...
public class OnlineMapsMarker3D : OnlineMapsMarkerBase
{
    ...
    public void Update(Bounds bounds, double tlx, double tly, double brx, double bry, int zoom, double ttlx, double ttly, double tbrx, double tbry, float bestYScale)
    {
        ...
        if (borderTransform || sizeType == SizeType.meters)
        {
            double dx, dy;
            ...
            if (borderTransform.gameObject.activeInHierarchy)
            {
                //Debug.Log($"scale: {scale:F2} -> {1.2f / scale:F2}");
                double d = (dx + dy) / 2 * (1.2f /scale);
                float fd = instance.transform.localScale.x / (float)d; 
                //Debug.Log($"{(float)d:F2} -> {fd:F2}");
                //Debug.Log("IGNORE SCALING BORDER FOR NOW");
                // borderTransform.localScale = Vector3.one * fd;
            }
        }

        if (oldPosition != newPosition)
        {
            //if (updateMarker) markerGroup.RecalculateMean(true);
